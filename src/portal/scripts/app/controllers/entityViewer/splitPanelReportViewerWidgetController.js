...
const SplitPanelExchangeService = require('../../services/groupTable/exchangeWithSplitPanelService');
...
export default function ($scope, $uiRouterGlobals, metaContentTypesService) {
    ...
    Object.keys(layoutData).forEach(key => {
        ...
        if ( !layoutData[key] ) {
            console.error("Error: not enough data for split panel from query parameters", $uiRouterGlobals.params);
    ...
    const additions = {
        ...
        type: metaContentTypesService.findEntityByContentType(layoutData.content_type),
    };
    ...
    parentWindow.postMessage({ action: 'SPLIT_WIDGET_INITIALIZED' }, 'http://0.0.0.0:8080');
