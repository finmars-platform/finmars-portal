...
export default function InstrmentTypePricingTabController($scope, $mdDialog) {
		...
		instrumentTypeService.updatePricing(vm.entity.id, item).then(function (data) {
			...
			$mdDialog.show({
				...
				locals: {
					info: {
						title: 'Success',
						description: "New Pricing Settings were applied" + JSON.stringify(data, null, 4)
					}
				}
			});
	...
	const generateInstrumentAttributeTypesByValueTypes = function () {
		...
		vm.attributeTypesByValueTypes[10] = vm.attributeTypesByValueTypes[10].concat(getAttributesByValueType(10));
		...
		vm.attributeTypesByValueTypes[20] = vm.attributeTypesByValueTypes[20].concat(getAttributesByValueType(20));
