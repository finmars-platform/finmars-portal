/**
 * Created by szhitenev on 30.05.2016. Form Editor Andrew
 */
(function () {
  "use strict";
  ...
  module.exports = function entityDataConstructorDialogController(
    $scope,
    data,
    $stateParams,
    $state,
      ...
      if (Object.keys(vm.fieldsTree).length) {
        Object.keys(vm.fieldsTree).forEach(function (tabNumber, tabIndex) {
          ...
          fixTab(
            ...
            numberOfRows,
            numberOfCols,
            vm.tabs[tabIndex].layout.fields
          );
        });
      }
              ...
              for (i = 0; i < tab.layout.fields.length; i = i + 1) {
                for (r = 0; r < tab.layout.rows; r = r + 1) {
                  for (c = columns; c < tab.layout.columns; c = c + 1) {
                    if (
                      tab.layout.fields[i].row == r + 1 &&
                      tab.layout.fields[i].column == c + 1
                    ) {
      ...
      return new Promise(function (resolve, reject) {
        attributeTypeService
          ...
          .then(function (data) {
            ...
            var keysOfFixedFieldsAttrs = metaService.getEntityViewerFixedFieldsAttributes(
              vm.entityType
            );
    ...
    vm.createFixedAreaFieldsTree = function () {
      var fixedAreaFields = JSON.parse(
        JSON.stringify(vm.fixedArea.layout.fields)
      );
      vm.fixedAreaFieldsTree = {};
    ...
    var onDropFromSocket = function (
      ...
      for (i = 0; i < draggedFromTab.layout.fields.length; i++) {
        ...
        if (
          field.column === draggedFromColumn &&
          field.row === draggedFromRow
        ) {
          var emptyFieldData = {
            colspan: 1,
    ...
    vm.openFormPreview = function ($event) {
      ...
      if (vm.entityType === "complex-transaction") {
        previewController =
          "ComplexTransactionFormsPreviewDialogController as vm";
