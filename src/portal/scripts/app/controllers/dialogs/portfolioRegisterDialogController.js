...
export default function PortfolioRegisterDialogController ($scope, $mdDialog, data) {
	...
	if (vm.selPortfolioData !== undefined) { // select portfolio inside portfolioRegisterDialogController
	...
	const init = function () {
		...
		const iTypeProm = new Promise((resolve, reject) => {
			...
			metaService.loadDataFromAllPages(instrumentTypeService.getListLight, [options]).then(instrTypesList => {
		...
		const ppProm = new Promise((resolve, reject) => {
			...
			metaService.loadDataFromAllPages(pricingPolicyService.getListLight, [options]).then(ppList => {
	...
	setTimeout(function () {
		vm.dialogElemToResize = document.querySelector('.twoInputsDialogElemToResize');
		$scope.$apply();
	}, 100);
