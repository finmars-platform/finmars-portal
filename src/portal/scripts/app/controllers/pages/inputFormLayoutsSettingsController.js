...
(function () {
    ...
    module.exports = function ($scope, $mdDialog, metaContentTypesService, uiService) {
        ...
        var getLayoutsByEntity = function (contentType) {
            ...
            var layoutsPropertyName = getLayoutDataByContentType(contentType, 'propertyName');
            ...
            var getLayoutsByEntityMethod = function (resolve, reject) {
                ...
                uiService.getListEditLayout(entityType, options).then(function (data) {
        ...
        vm.setAttributeManagerContainersHeight = function () { // for collapse animation
        ...
        vm.setAsDefault = function (layoutToMakeDefault) {
        	...
        	if (!layoutToMakeDefault.is_default) {
                ...
                return uiService.updateEditLayout(layoutToMakeDefault.id, layoutToMakeDefault);
            }
                ...
                if (res.status === 'agree') {
                    ...
                    uiService.deleteEditLayoutByKey(layout.id).then(function (data) {
                        ...
                        getLayoutsByEntity(layout.content_type).then(function () {
                            ...
                            if (layout.is_default) { // if default layout was deleted set another layout as default

                                var layoutsPropertyName = getLayoutDataByContentType(layout.content_type, 'propertyName');
                                var newDefaultLayout = null;

                                if (vm[layoutsPropertyName] && vm[layoutsPropertyName].length > 0) {
                                    newDefaultLayout = vm[layoutsPropertyName][0];
                                }

                                newDefaultLayout.is_default = true;
                                uiService.updateEditLayout(newDefaultLayout.id, newDefaultLayout);
