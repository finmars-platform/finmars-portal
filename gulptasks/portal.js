...
(function () {
  ...
  gulp.task(
    appName + "-js-min",
    gulp.series(appName + "-HTML-to-JS", function () {
      ...
      return (
        browserify(pathToJS)
          ...
          .pipe(preprocess())
          .pipe(
            gulpif(PROJECT_ENV === "local", replace(/__API_HOST__/g, API_HOST))
          )
  ...
  gulp.task(
    appName + "-js-min-All",
    gulp.series(appName + "-HTML-to-JS", appName + "-js-min")
  );
  ...
  gulp.task(appName + "-watch-All", function () {
    ...
    gulp.watch(
      "src/" + appName + "/**/*.less",
      gulp.series(appName + "-less-to-css-min")
    );
    gulp.watch(
      "src/" + appName + "/**/*.js",
      gulp.series(appName + "-only-js-min")
    );
    gulp.watch(
      "src/" + appName + "/**/*.html",
      gulp.series(appName + "-HTML-to-JS")
    );
  ...
  gulp.task("forum-watch-All", function () {
    ...
    gulp.watch(
      "src/forum/**/*.html",
      gulp.series("portal-forum-HTML-to-JS", appName + "-js-min")
    );
  });
